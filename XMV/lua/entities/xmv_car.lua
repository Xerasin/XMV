...
function ENT:OnMove(ply, data)
	...
	if(phys and phys:IsValid()) then
		...
		if(hit) then
			...
			local ang_dif = ang - ply:EyeAngles() - Angle(0, 90, 0)
			local yaw = math.floor((ang_dif.yaw % 360))
			--eprint(yaw)
		...
		--[[if trace.Hit then
			...
			phys:AddVelocity(trace.HitNormal * phys:GetMass() * -1)
		else
...
if(CLIENT) then
	...
	function ENT:CreateModels()
		local box = ClientsideModel("models/props_junk/cardboard_box001a.mdl", RENDERGROUP_OPAQUE)
		box:SetNoDraw(true)
		local mat = Matrix()
		...
		local wheel = ClientsideModel("models/props_vehicles/carparts_wheel01a.mdl", RENDERGROUP_OPAQUE)
		local mat = Matrix()
		mat:Scale(Vector(0.15,0.15,0.15))
